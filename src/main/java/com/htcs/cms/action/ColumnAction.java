...
import com.htcs.cms.biz.IColumnBiz;
import com.htcs.cms.entity.ColumnEntity;
...
public class ColumnAction extends BaseAction{
	...
	public String index(HttpServletResponse response,HttpServletRequest request){
        boolean isDoctor = Boolean.valueOf(request.getParameter("isDoctor"));
        request.setAttribute("isDoctor", isDoctor);
        if (isDoctor) {
            return view("/doctorCategory/index");
        } else {
	...
	public String add(HttpServletRequest request,ModelMap model) {
        ...
        List<ColumnEntity> list = columnBiz.queryAll(appId, this.getModelCodeId(request), isDoctor);
        ColumnEntity columnSuper = new ColumnEntity();
        ColumnEntity columnEntity = new ColumnEntity();
        columnEntity.setCategoryCategoryId(155);
        model.addAttribute("appId", appId);
        model.addAttribute("columnSuper", columnSuper);
        model.addAttribute("column", columnEntity);
        model.addAttribute("listColumn", JSONArray.toJSONString(list));
        if (isDoctor) {
            return view("/doctorCategory/form");
        } else {
	...
	private void columnPath(HttpServletRequest request,ColumnEntity column){
		...
		if(column.getCategoryCategoryId() == 0){
			...
			List<com.mingsoft.basic.entity.ColumnEntity> list = columnBiz.queryParentColumnByColumnId(column.getCategoryId());
			if(!StringUtil.isBlank(list)){
				String temp = "";
				for(int i = list.size()-1; i>=0; i--){
					com.mingsoft.basic.entity.ColumnEntity entity = list.get(i);
	 ...
	 * @param column  栏目实体
	 */
	private void columnPath(HttpServletRequest request, com.mingsoft.basic.entity.ColumnEntity column) {
		String columnPath = "";
		String file = this.getRealPath(request, null) + IParserRegexConstant.HTML_SAVE_PATH + File.separator + column.getAppId();
		String delFile = "";
		//修改栏目路径时，删除已存在的文件夹
	...
	public String edit(@PathVariable int columnId, HttpServletRequest request,ModelMap model) {
		...
		// 判断管理员权限,查询其管理的栏目集合
        list = columnBiz.queryAll(appId, this.getModelCodeId(request), isDoctor);
        //查询当前栏目实体
	...
	public void list(@ModelAttribute ColumnEntity column,HttpServletResponse response, HttpServletRequest request,ModelMap model) {
		...
		// 需要打开的栏目节点树的栏目ID
        List list = columnBiz.queryAll(websiteId, this.getModelCodeId(request), isDoctor);
	...
	public void update(@ModelAttribute ColumnEntity column,HttpServletRequest request,HttpServletResponse response) {
		...
		this.columnPath(request,column);
		//查询当前栏目是否有子栏目，
		List<com.mingsoft.basic.entity.ColumnEntity> childList = columnBiz.queryChild(column.getCategoryId(), websiteId, this.getModelCodeId(request),null);
